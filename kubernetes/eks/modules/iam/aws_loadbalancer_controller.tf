resource "aws_iam_policy" "aws_loadbalancer_controller" {
  ...
  description = "Allows AWS Loadbalancer Controller to modify Route53 to support Lets Encrypt ACME DNS01."
...
resource "aws_iam_role_policy_attachment" "aws_loadbalancer_controller" {
  count      = var.enable_aws_loadbalancer_controller ? 1 : 0
  role       = aws_iam_role.aws_loadbalancer_controller[0].name
  policy_arn = aws_iam_policy.aws_loadbalancer_controller[0].arn
}
